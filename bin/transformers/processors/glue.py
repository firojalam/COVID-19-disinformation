...
from .texttokenize import tokenize
from .tokenize_n_normalize_tweets import tokenize_text
...
def glue_convert_pred_examples_to_features(
    ...
    if is_tf_available() and isinstance(examples, tf.data.Dataset):
        if task is None:
            raise ValueError("When calling glue_convert_examples_to_features from TF, the task parameter is required.")
        return _tf_glue_convert_examples_to_features(examples, tokenizer, max_length=max_length, task=task)
    return _glue_convert_pred_examples_to_features(
        examples, tokenizer, max_length=max_length, task=task, label_list=label_list, output_mode=output_mode
    )
...
def _glue_convert_pred_examples_to_features(
    examples: List[InputExample],
    tokenizer: PreTrainedTokenizer,
    max_length: Optional[int] = None,
    task=None,
...
class MultiClassProcessor(DataProcessor):
    ...
    def set_train_file(self,file_name):
        ...
        return self.get_train_examples(file_name)

    def set_dev_file(self,file_name):
        self.dev_file = file_name
        return self.get_dev_examples(file_name)

    def set_test_file(self,file_name):
        self.test_file = file_name
        return self.get_test_examples(file_name)
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            text_a = line[1]
            if(self.lang=="arabic"):
                text_a = tokenize_text(text_a)
            else:
                text_a = tokenize(text_a)
            label = line[self.label_index].strip()
...
class MultiClassProcessorOrg(DataProcessor):
    """Processor for the Multiclass classification. automatically calculated number of labels"""
    ...
    def get_train_examples(self, file_name):
        """See base class."""
        return self._create_examples(self._read_tsv(os.path.join(file_name, "train.tsv")), "train")

    def get_dev_examples(self, file_name):
        """See base class."""

        return self._create_examples(self._read_tsv(os.path.join(file_name, "dev.tsv")), "dev")

    def get_test_examples(self, file_name):
        """See base class."""
        return self._create_examples(self._read_tsv(os.path.join(file_name, "test.tsv")),"test")

    def get_predict_examples(self, file_name):
        """See base class."""
        # logger.info("file name %s", file_name)
        return self._create_predict_examples(self._read_tsv(file_name),"predict")
